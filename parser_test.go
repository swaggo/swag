...
`
    ...
    "definitions": {
		...
		p := New()
		_ = p.packages.CollectAstFile("api", "api/api.go", f, NewLogger())
...
func TestParseTypeOverrides(t *testing.T) {
	...
	p := New(SetOverrides(map[string]string{
		"github.com/extrame/swag/testdata/global_override/types.Application":  "string",
		"github.com/extrame/swag/testdata/global_override/types.Application2": "github.com/extrame/swag/testdata/global_override/othertypes.Application",
		"github.com/extrame/swag/testdata/global_override/types.ShouldSkip":   "",
	}))
	err := p.ParseAPI(searchDir, mainAPIFile, defaultParseDepth)
	assert.NoError(t, err)
...
`
   ...
   "api.Response": {
	...
	f, err := goparser.ParseFile(token.NewFileSet(), "", src, goparser.ParseComments)
	assert.NoError(t, err)
	_ = parser.packages.CollectAstFile("api", "api/api.go", f, NewLogger())

	f2, err := goparser.ParseFile(token.NewFileSet(), "", restsrc, goparser.ParseComments)
	assert.NoError(t, err)
	_ = parser.packages.CollectAstFile("rest", "rest/rest.go", f2, NewLogger())
...
`
	...
	_ = pkgs.CollectAstFile("api", "api/api.go", f, NewLogger())
