...
func TestParseEmptyComment(t *testing.T) {
	...
	err := operation.ParseComment("//", nil, NewLogger())
...
func TestParseTagsComment(t *testing.T) {
	...
	operation := NewOperation(nil)
	err := operation.ParseComment(`/@Tags pet, store,user`, nil, NewLogger())
	assert.NoError(t, err)
	assert.Equal(t, operation.basic_operation.Tags, []string{"pet", "store", "user"})
}
...
func TestParseAcceptComment(t *testing.T) {
	...
	operation := NewOperation(nil)
	err := operation.ParseComment(comment, nil, NewLogger())
	assert.NoError(t, err)
	assert.Equal(t,
		operation.basic_operation.Consumes,
...
func TestParseProduceCommentErr(t *testing.T) {
	...
	operation := new(Operation)
	err := operation.ParseComment("/@Produce foo", nil, NewLogger())
	assert.Error(t, err)
}
...
func TestParseRouterMultipleComments(t *testing.T) {
	...
	err = operation.ParseComment(anotherComment, nil, NewLogger())
	assert.NoError(t, err)
...
func TestOperation_ParseResponseWithDefault(t *testing.T) {
	...
	assert.Equal(t, "An empty response", operation.basic_operation.Responses.Default.Description)
	...
	assert.Equal(t, "A response", operation.basic_operation.Responses.Default.Description)
	assert.Equal(t, "A response", operation.basic_operation.Responses.StatusCodeResponses[200].Description)
}
...
func TestParseResponseSuccessCommentWithEmptyResponse(t *testing.T) {
	...
	response := operation.basic_operation.Responses.StatusCodeResponses[200]
...
func TestParseResponseCommentWithObjectTypeInSameFile(t *testing.T) {
	...
	err = operation.ParseComment(comment, astFile, NewLogger())
	assert.NoError(t, err)
...
func TestParseResponseCommentWithHeader(t *testing.T) {
	...
	operation := NewOperation(nil)
	err := operation.ParseComment(`@Success 200 "it's ok"`, nil, NewLogger())
	assert.NoError(t, err, "ParseComment should not fail")

	err = operation.ParseComment(`@Header 200 {string} Token "qwerty"`, nil, NewLogger())
	assert.NoError(t, err, "ParseComment should not fail")
	...
	err = operation.ParseComment(`@Header 200 "Mallformed"`, nil, NewLogger())
	assert.Error(t, err, "ParseComment should not fail")

	err = operation.ParseComment(`@Header 200,asdsd {string} Token "qwerty"`, nil, NewLogger())
	assert.Error(t, err, "ParseComment should not fail")
}
...
func TestParseEmptyResponseOnlyCode(t *testing.T) {
	...
	operation := NewOperation(nil)
	err := operation.ParseComment(`@Success 200`, nil, NewLogger())
	assert.NoError(t, err, "ParseComment should not fail")
...
func TestParseResponseCommentParamMissing(t *testing.T) {
	...
	paramLenErr := operation.ParseComment(paramLenErrComment, nil, NewLogger())
...
func TestOperation_ParseParamComment(t *testing.T) {
	...
	t.Run("integer", func(t *testing.T) {
		...
		for _, paramType := range []string{"header", "path", "query", "formData"} {
			t.Run(paramType, func(t *testing.T) {
				o := NewOperation(nil)
				err := o.ParseComment(`@Param some_id `+paramType+` int true "Some ID"`, nil, NewLogger())

				assert.NoError(t, err)
				assert.Equal(t, o.basic_operation.Parameters, []spec.Parameter{{
	...
	t.Run("string", func(t *testing.T) {
		...
		for _, paramType := range []string{"header", "path", "query", "formData"} {
			t.Run(paramType, func(t *testing.T) {
				o := NewOperation(nil)
				err := o.ParseComment(`@Param some_string `+paramType+` string true "Some String"`, nil, NewLogger())
	...
	t.Run("object", func(t *testing.T) {
		...
		for _, paramType := range []string{"header", "path", "query", "formData"} {
			t.Run(paramType, func(t *testing.T) {
				assert.Error(t, NewOperation(nil).ParseComment(`@Param some_object `+paramType+` main.Object true "Some Object"`, nil, NewLogger()))
			})
		}
	})
...
func TestParseParamCommentBodyArray(t *testing.T) {
	...
	o := NewOperation(nil)
	err := o.ParseComment(comment, nil, NewLogger())
	assert.NoError(t, err)
	assert.Equal(t, o.basic_operation.Parameters, []spec.Parameter{{
		ParamProps: spec.ParamProps{
			Name:        "names",
...
func TestParseParamCommentArray(t *testing.T) {
	...
	for _, paramType := range paramTypes {
		t.Run(paramType, func(t *testing.T) {
			operation := NewOperation(nil)
			err := operation.ParseComment(`@Param names `+paramType+` []string true "Users List"`, nil, NewLogger())

			assert.NoError(t, err)

			b, _ := json.MarshalIndent(operation.basic_operation.Parameters, "", "    ")
			expected := `[
    {
        "type": "array",
        "items": {
        ...
        "required": true
			...
			err = operation.ParseComment(`@Param names `+paramType+` []model.User true "Users List"`, nil, NewLogger())
			assert.Error(t, err)
		})
	}
}
...
func TestParseParamCommentDefaultValue(t *testing.T) {
	...
	operation := NewOperation(nil)
	err := operation.ParseComment(`@Param names query string true "Users List" default(test)`, nil, NewLogger())
	assert.NoError(t, err)
...
func TestParseParamCommentByBodyTypeWithDeepNestedFields(t *testing.T) {
	...
	err := operation.ParseComment(comment, nil, NewLogger())
	assert.NoError(t, err)
	assert.Len(t, operation.basic_operation.Parameters, 1)
	assert.Equal(t, "test deep", operation.basic_operation.Parameters[0].Description)
	assert.True(t, operation.basic_operation.Parameters[0].Required)

	b, err := json.MarshalIndent(operation.basic_operation.Parameters, "", "    ")
	assert.NoError(t, err)
	expected := `[
    {
...
func TestParseParamCommentByEnums(t *testing.T) {
	...
	assert.Error(t, operation.ParseComment(comment, nil, NewLogger()))
...
func TestParseParamCommentByMinimum(t *testing.T) {
	...
	assert.NoError(t, operation.ParseComment(comment, nil, NewLogger()))
...
func TestParseIdComment(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, "myOperationId", operation.basic_operation.ID)
}
...
func TestParseSecurityComment(t *testing.T) {
	...
	assert.Equal(t, operation.basic_operation.Security, []map[string][]string{
		{
...
func TestParseDeprecationDescription(t *testing.T) {
	...
	if !operation.basic_operation.Deprecated {
...
func TestParseExtentions(t *testing.T) {
	...
	// OK
	{
		...
		err := operation.ParseComment(comment, nil, NewLogger())
		assert.NoError(t, err)
		assert.Equal(t, operation.basic_operation.Extensions["x-amazon-apigateway-integration"],
	...
	// Test x-tagGroups
	{
		...
		err := operation.ParseComment(comment, nil, NewLogger())
		assert.NoError(t, err)
		assert.Equal(t, operation.basic_operation.Extensions["x-tagGroups"],
...
func TestParseResponseHeaderComment(t *testing.T) {
	...
	operation := NewOperation(nil)
	operation.basic_operation.Responses = &spec.Responses{}
	err := operation.ParseResponseComment(`default {string} string "other error"`, nil, NewLogger())
	assert.NoError(t, err)
...
func TestParseCodeSamples(t *testing.T) {
	...
	t.Run("Find sample by file", func(t *testing.T) {
		...
		operation.basic_operation.Summary = "example"

		err := operation.ParseComment(comment, nil, NewLogger())
		assert.NoError(t, err, "no error should be thrown")
		assert.Equal(t, operation.basic_operation.Summary, "example")
		assert.Equal(t, operation.basic_operation.Extensions["x-codeSamples"],
			map[string]interface{}{"lang": "JavaScript", "source": "console.log('Hello World');"})
	})

	t.Run("With broken file sample", func(t *testing.T) {
		operation := NewOperation(nil, SetCodeExampleFilesDirectory("testdata/code_examples"))
		operation.basic_operation.Summary = "broken"
	...
	t.Run("Example file not found", func(t *testing.T) {
		operation := NewOperation(nil, SetCodeExampleFilesDirectory("testdata/code_examples"))
		operation.basic_operation.Summary = "badExample"
	...
	t.Run(" broken dir", func(t *testing.T) {
		...
		operation.basic_operation.Summary = "code"
